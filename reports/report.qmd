...
SELECT `Short Name`
FROM world_bank_data.wdi_country
WHERE `Region` IS NULL OR `Income Group` IS NULL
   OR `Short Name` IN ('World', 'High income', 'Upper middle income', 'Lower middle income', 'Low income');
""")
...
```{python}
run_sql_and_return_html(cnx, """
SELECT COUNT(*) AS Low_Income_Count, `Region`
FROM wdi_country
...
SELECT `Region`, `Income Group`,
  ...
  (SELECT COUNT(*) FROM wdi_country WHERE `Region` = w.`Region`) AS Region_Total,
  (SELECT COUNT(*) FROM wdi_country WHERE `Income Group` = w.`Income Group`) AS Income_Total,
...
run_sql_and_return_html(cnx, """
SELECT `Region`,
  ROUND(100 * SUM(CASE WHEN `Income Group` = 'High income' THEN 1 ELSE 0 END) / COUNT(*), 2) AS High_income_pct,
  ROUND(100 * SUM(CASE WHEN `Income Group` = 'Upper middle income' THEN 1 ELSE 0 END) / COUNT(*), 2) AS Upper_middle_pct,
  ROUND(100 * SUM(CASE WHEN `Income Group` = 'Lower middle income' THEN 1 ELSE 0 END) / COUNT(*), 2) AS Lower_middle_pct,
  ROUND(100 * SUM(CASE WHEN `Income Group` = 'Low income' THEN 1 ELSE 0 END) / COUNT(*), 2) AS Low_income_pct
FROM wdi_country
GROUP BY `Region`
ORDER BY `Region`;
""")
